...
<style>
    .stApp {{
        background:
            radial-gradient(circle at 95% 5%, rgba(0,180,216,0.08), transparent 28%),
    ...
    .recommendation-header {{
        padding: 18px 22px;
        border-radius: 16px;
        background: linear-gradient(135deg, #F5FCFF, #EAF7FF);
...
try:
    ...
    st.error(
        ...
        "Confirm that all CSV, PKL and logo files are in the correct folders."
    )
    st.stop()
...
<div class="hero">
    ...
    <p>
        ...
        HealthPulse AI combines health risk scoring, system readiness,
        machine learning and actionable recommendations to identify where
        intervention is needed before a health crisis escalates.
    </p>
</div>
""",
        unsafe_allow_html=True,
...
def metric_card(label: str, value, note: str = "") -> None:
    st.markdown(
        f"""
...
def readable_name(column: str) -> str:
    custom_names = {
        ...
        "under_5_mortality_rate": "Under-5 mortality rate",
    }
...
def style_plotly(
    ...
    fig.update_layout(
        ...
        margin={
            "l": 30,
            "r": 35,
            "t": 105 if legend_position == "top" else 75,
            "b": 80 if legend_position == "bottom" else 45,
        },
...
def predict_next_risk(selected_row: pd.DataFrame):
    prediction_data = selected_row[final_features].copy()

    predicted_code = model.predict(prediction_data)[0]
    predicted_level = reverse_risk_mapping[int(predicted_code)]

    predicted_probabilities = model.predict_proba(prediction_data)[0]
    ...
    probability_dictionary = {
        class_names[index]: float(predicted_probabilities[index])
...
def identify_concerns(row: pd.Series, top_n: int = 5):
    concerns = []
    ...
    for indicator in RISK_INDICATORS:
        ...
        if value > median_value:
            relative_gap = (
                (value - median_value) / abs(median_value)
                if median_value != 0
            ...
            concerns.append(
                {
                    "indicator": indicator,
                    "message": (
                        f"{readable_name(indicator)} is above "
                        "the ASEAN median."
                    ),
    ...
    for indicator in READINESS_INDICATORS:
        ...
        if value < median_value:
            relative_gap = (
                (median_value - value) / abs(median_value)
                if median_value != 0
                else median_value - value
            )
            ...
            concerns.append(
                {
                    "indicator": indicator,
                    "message": (
                        f"{readable_name(indicator)} is below "
                        "the ASEAN median."
                    ),
                    "severity": relative_gap,
                }
...
def generate_actions(concerns):
    ...
    if {
        ...
        actions.append(
            "Expand child health services, immunization outreach, "
            "early disease screening and community child-survival programmes."
        )

    if "maternal_mortality_rate" in indicators:
        actions.append(
            "Strengthen emergency obstetric care, skilled birth attendance, "
            "maternal referral systems and midwife-led services."
        )

    if "malaria_prevalence" in indicators:
        actions.append(
            "Expand malaria surveillance, rapid testing, vector control "
            "and community prevention campaigns."
        )

    if "tb_prevalence" in indicators:
        actions.append(
            "Scale up TB screening, early case detection, treatment-adherence "
            "support and community monitoring."
        )

    if "undernourished_population" in indicators:
        actions.append(
            "Integrate nutrition support with maternal and child health "
    ...
    if {
        ...
        actions.append(
            "Launch targeted vaccine catch-up campaigns and strengthen "
            "routine immunization delivery."
        )

    if "nurses_midwives_density" in indicators:
        actions.append(
            "Increase the deployment and retention of nurses and midwives "
            "in underserved and rural facilities."
        )

    if "physicians_density" in indicators:
        actions.append(
            "Improve physician access using mobile clinics, telemedicine "
            "and rural-service incentives."
        )

    if "government_health_expenditure" in indicators:
        actions.append(
            "Increase targeted public health financing for frontline services "
            "and high-burden health priorities."
        )

    if not actions:
        actions.append(
            "Maintain current health investments, strengthen surveillance "
            "and continue monitoring early-warning indicators."
        )
...
def render_recommendation(
    ...
    probability_high = probability_dictionary.get("High", 0.0)
...
""",
    ...
    with st.container(border=True):
        ...
        with summary_col1:
            st.markdown(f"**Country assessed:** {row['country']}")
            st.markdown(f"**Assessment year:** {int(row['year'])}")

        with summary_col2:
            st.markdown(
                f"**Predicted next-year risk:** {predicted_level}"
            )
            st.markdown(
                f"**Probability of high risk:** {probability_high:.1%}"
            )
        ...
        if concerns:
            for concern in concerns:
                st.markdown(f"- {concern['message']}")
        else:
            st.success(
                "No major indicator exceeded the current ASEAN "
                "risk and readiness reference thresholds."
            )

        st.markdown("#### Recommended actions")

        for action_number, action in enumerate(actions, start=1):
            st.markdown(f"{action_number}. {action}")
...
<div style="font-size:13px;opacity:0.85;margin-bottom:18px;">
...
if page == "Executive Overview":
    ...
    average_risk = latest_df["health_risk_score"].mean()
    average_readiness = latest_df["readiness_score"].mean()

    high_risk_count = (
        latest_df["health_risk_level"] == "High"
    ).sum()

    emergency_count = (
        latest_df["priority_category"] == "Emergency Priority"
    ).sum()
    ...
    with col2:
        metric_card(
            "Historical period",
            f"{health_df['year'].min()}–{health_df['year'].max()}",
    ...
    with col4:
        metric_card(
            "Emergency cases",
            int(emergency_count),
            f"Emergency-priority countries in {latest_year}",
        )
    ...
    section_header(
        ...
        (
            f"Countries ranked by their composite "
            f"Health Risk Score in {latest_year}."
        ),
    )
    ...
    risk_bar = px.bar(
        ...
        category_orders={
            "health_risk_level": ["High", "Medium", "Low"]
        },
        labels={
            "health_risk_score": "Health risk score",
            "country": "",
            "health_risk_level": "Risk level",
        },
    )
    ...
    risk_bar.update_traces(
        ...
        hovertemplate=(
            "<b>%{y}</b><br>"
            "Health risk score: %{x:.1f}<extra></extra>"
        ),
    )
    ...
    risk_bar.update_xaxes(
        ...
        range=[
            0,
            ranked_latest["health_risk_score"].max() * 1.16,
        ],
    )

    risk_bar.update_yaxes(title=None)

    risk_bar = style_plotly(
        risk_bar,
        f"Health Risk Scores Across ASEAN Countries, {latest_year}",
        height=570,
        legend_position="top",
    )
    ...
    section_header(
        ...
        (
            "Share of ASEAN countries classified as Low, "
            "Medium or High risk over time."
        ),
    )
    ...
    area_chart = style_plotly(
        area_chart,
        "Health Risk Levels Across ASEAN, 2004–2014",
        height=480,
        legend_position="top",
    )
...
# ============================================================
    ...
    section_header(
        "Risk Intelligence",
        (
            "Explore the countries carrying the greatest health burden "
            "and compare changes across time."
        ),
    )
    ...
    selected_year = st.selectbox(
        ...
        index=len(health_df["year"].unique()) - 1,
    )
    ...
    with left_column:
        ...
        year_bar.update_xaxes(
            range=[
                0,
                sorted_year_df["health_risk_score"].max() * 1.18,
            ]
        )
        ...
        year_bar = style_plotly(
            year_bar,
            f"Health Risk Scores by Country, {selected_year}",
            height=565,
            legend_position="top",
        )
    ...
    with right_column:
        ...
        risk_donut.add_annotation(
            text=f"<b>{len(year_df)}</b><br>countries",
            x=0.5,
            y=0.5,
            showarrow=False,
            font={
        ...
        risk_donut = style_plotly(
            risk_donut,
            f"Risk Category Mix, {selected_year}",
            height=565,
    ...
    section_header(
        "Country Risk Trends",
        "Select countries to compare their Health Risk Scores over time.",
    )
    ...
    if selected_countries:
        ...
        st.info(
            "Select at least one country to display the trend chart."
        )
...
# ============================================================
    ...
    section_header(
        ...
        (
            "Identify countries combining a high health burden "
            "with weak health-system capacity."
        ),
    )
    ...
    with left_column:
        matrix_chart = px.scatter(
            ...
            size_max=35,
            labels={
                "readiness_score": "Health-system readiness score",
                "health_risk_score": "Health risk score",
        ...
        matrix_chart = style_plotly(
            matrix_chart,
            f"Risk–Readiness Priority Matrix, {selected_year}",
            height=600,
            legend_position="bottom",
        )
    ...
    with right_column:
        ...
        priority_bar.update_xaxes(
            range=[
                0,
                max(priority_counts["count"].max() * 1.35, 1),
            ]
        )

        priority_bar.update_yaxes(title=None)
        ...
        priority_bar.update_layout(showlegend=False)
    ...
    section_header(
        "Priority Table",
        "Country results sorted from higher risk to weaker readiness.",
    )
...
# ============================================================
    ...
    section_header(
        "AI Prediction Copilot",
        (
            "Use current-year health indicators to predict "
            "the following year's risk level and recommended response."
        ),
    )
    ...
    with selection_column:
        with st.container(border=True):
            ...
            st.caption(
                "The model uses the selected year's health indicators "
                "to estimate the following year's risk category."
            )

    if selected_row.empty:
        st.warning(
            "No data is available for the selected country and year."
        )

    else:
        predicted_level, probability_dictionary = (
            predict_next_risk(selected_row)
        )

        selected_series = selected_row.iloc[0]
        ...
        with result_column:
            ...
            with result_col1:
                metric_card(
                    "Current risk",
                    selected_series["health_risk_level"],
                    (
                        f"Score: "
            ...
            with result_col2:
                metric_card(
                    "Current readiness",
                    selected_series["readiness_level"],
                    (
                        f"Score: "
        ...
        probability_df = pd.DataFrame(
            {
                ...
                "Probability": [
                    probability_dictionary.get("Low", 0),
                    probability_dictionary.get("Medium", 0),
                    probability_dictionary.get("High", 0),
                ],
            }
        )
        ...
        probability_chart = style_plotly(
            probability_chart,
            "Next-Year Risk Prediction Probability",
            height=430,
            legend_position="none",
        )

        probability_chart.update_layout(showlegend=False)
...
# ============================================================
    ...
    section_header(
        "Country Deep Dive",
        (
            "Explore one country's risk, readiness and "
            "priority pathway across the historical period."
        ),
    )
    ...
    with col3:
        metric_card(
            ...
            f"Assessment year: {latest_row['year']}",
        )
    ...
    trend_chart.add_trace(
        go.Scatter(
            ...
            hovertemplate=(
                "Year: %{x}<br>"
                "Readiness score: %{y:.1f}<extra></extra>"
            ),
        )
    )
    ...
    trend_chart = style_plotly(
        trend_chart,
        f"Health Risk and Readiness Trend: {selected_country}",
        height=530,
        legend_position="top",
    )
    ...
    section_header(
        ...
        "Historical score and priority classification.",
    )
...
# ============================================================
    ...
    section_header(
        "Methodology",
        "How HealthPulse AI converts fragmented indicators into decision intelligence.",
    )
...
- 2013 indicators predict the 2014 risk level
...
""",
    ...
    render_method_step(
        ...
        "Data Limitation",
        """
The historical dataset covers **10 ASEAN countries from 2004 to 2014**.
